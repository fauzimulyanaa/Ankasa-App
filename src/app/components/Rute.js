...
function Rute() {
  ...
  const getTickets = async () => {
    try {
      ...
      setTicketData(data);
      setDepartureCity(data.from.location);
      setDestinationCity(data.to.location);
  ...
  const handleSearch = () => {
    // Filter tiket berdasarkan kota keberangkatan dan tujuan yang dipilih
              ...
              <div className="detail flex items-center justify-center gap-10">
                ...
                <div className="to">
                  ...
                  <select className="text-[16px] font-[600] text-white bg-transparent border-none focus:outline-none" value={destinationCity} onChange={(e) => setDestinationCity(e.target.value)}>
                    {/* Render dropdown options based on available cities */}
                    {ticketData.map((ticket) => (
                ...
                <button className="bg-[#F7931E] text-white px-4 py-2 rounded-lg" onClick={handleSearch}>
                  Search
                </button>
              </div>
            </div>
